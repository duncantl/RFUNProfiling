...
void
showProfStack(RCNTXT *ctxt)
{
	    Rprintf("<stack>\n");
		    while(ctxt && ctxt->call != R_NilValue) {
			if(ctxt->call)
			    Rf_PrintValue(ctxt->call);
			if(ctxt->callfun)
			    Rf_PrintValue(ctxt->callfun);
...
static void doprof(int sig)  /* sig is ignored in Windows */
{
	...
	if ((cptr->callflag & (CTXT_FUNCTION | CTXT_BUILTIN))
		...
		if (TYPEOF(fun) == SYMSXP) {
		    ...
		    if(cptr->nextcontext && cptr->nextcontext->callfun && 
		       TYPEOF(cptr->nextcontext->callfun) == SYMSXP && strcmp(CHAR(PRINTNAME(cptr->nextcontext->callfun)), "mapply") == 0) {
//   			   Rf_PrintValue(cptr->nextcontext->call);
			   sprintf(itembuf, "<mapply-Anonymous>");
		    } else {
//			showProfStack(cptr);
		...
		if(strcmp(itembuf, "FUN") == 0) { // don't need prevctxt!
//                    Rprintf("found FUN in profiling\n");
		    RCNTXT *ctxt = cptr;
		    SEXP arg2;
		    SEXP lcall;
		    int ok = 0;
		    ...
		    ctxt = cptr;
		    while(ctxt && ctxt->nextcontext) {
			lcall = ctxt->nextcontext->call;
			SEXP f = CAR(lcall);
			if(TYPEOF(f) == SYMSXP && strcmp(CHAR(PRINTNAME(f)), "vapply") == 0 || strcmp(CHAR(PRINTNAME(f)), "eapply") == 0 || strcmp(CHAR(PRINTNAME(f)), "apply") == 0  ) {
			    ok = 1;
			    break;
			} else if(TYPEOF(f) == SYMSXP && (strcmp(CHAR(PRINTNAME(f)), "sapply") == 0  || strcmp(CHAR(PRINTNAME(f)), "tapply") == 0 || strcmp(CHAR(PRINTNAME(f)), "by") == 0 || strcmp(CHAR(PRINTNAME(f)), "aggregate") == 0 || strcmp(CHAR(PRINTNAME(f)), "mapply") == 0 ))    {
//			    lcall = ctxt->nextcontext->nextcontext->call;
			    //lcall = ctxt->nextcontext->call;
			    ok = 1;
			    break;
			} else if(TYPEOF(f) == SYMSXP && strcmp(CHAR(PRINTNAME(f)), "lapply") == 0 ) {

			    arg2 = CADDR(lcall);
			    if(TYPEOF(arg2) == SYMSXP && strcmp(CHAR(PRINTNAME(arg2)), "FUN") != 0) {
				ok = 1;
				break;
			    }

/*
			    RCNTXT *tmp = ctxt->nextcontext;
			    if(tmp->nextcontext && tmp->nextcontext->call &&
			       TYPEOF(CAR(tmp->nextcontext->call)) == SYMSXP && 
			       strcmp(CHAR(PRINTNAME(CAR(tmp->nextcontext->call))), "sapply") == 0) {
				lcall = tmp->nextcontext->call;
				break;
			    }
...
*/
		    ...
		    if(ok) {
//			arg2 = CADDR(lcall);
			const char const *funName = CHAR(PRINTNAME(CAR(lcall)));
			if(strcmp(funName, "tapply") ==  0 || strcmp(funName, "by") ==  0)
			    arg2 = CADDDR(lcall);
			else if(strcmp(funName, "mapply") ==  0 )
			    arg2 = CADR(lcall);
		        else
   			    arg2 = CADDR(lcall);
...
//			Rf_PrintValue(arg2);
			if(TYPEOF(arg2) == SYMSXP)
   			   snprintf(itembuf, PROFITEMMAX-1, "%s", CHAR(PRINTNAME(arg2)));
//			else
//			    showProfStack(cptr);
			// Rf_PrintValue(cptr->nextcontext->call);
		    } else {
//			showProfStack(cptr); 	//	Rf_PrintValue(cptr->nextcontext->call);
		    }
                }
